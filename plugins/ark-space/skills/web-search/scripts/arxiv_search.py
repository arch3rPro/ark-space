...
from arkspace_runtime import provider_config  # noqa: E402
...
def classify_error(exc):
    """Map an arXiv fetch/parse exception to a (kind, status) failure tuple.

    arXiv has no provider configuration, so only request-time and response
    failures are classified. HTTP statuses go through :func:`classify_failure`;
    XML parse errors are treated as a malformed success body (``invalid-response``)
    and usage errors (a missing query) as ``invalid-request``.
    """
    if isinstance(exc, urllib.error.HTTPError):
        return provider_config.classify_failure(exc.code), exc.code
    ...
    if isinstance(exc, (TimeoutError, urllib.error.URLError, OSError)):
        return provider_config.classify_failure(None, str(exc)), None
    return "unknown", None
...
def main(argv=None):
    ...
    try:
        ...
        provider_config.write_error_file(
            "arxiv", "web_search", kind=kind, status=status, message=str(exc)
        )
