...
def fetch_json(url: str, timeout: float) -> Any:
    ...
    # ``url`` is built from the explicitly configured SearXNG endpoint plus encoded query parameters.
...
def classify_search_error(exc):
    """Map a SearXNG request/response exception to a (kind, status) tuple.

    Missing/configured-endpoint failures are handled separately as ``config``;
    here we classify endpoint and network failures plus malformed responses.
    """
    if isinstance(exc, urllib.error.HTTPError):
        return classify_failure(exc.code), exc.code
    if isinstance(exc, (urllib.error.URLError, TimeoutError)):
        return classify_failure(None, str(exc)), None
...
def main() -> int:
    ...
    try:
        ...
        write_error_file(PROVIDER_ID, CAPABILITY, kind="config", message=str(exc))
        print(
    ...
    if not bases:
        ...
        write_error_file(PROVIDER_ID, CAPABILITY, kind="config", message="no SearXNG endpoint configured")
        print(
    ...
    for base in bases:
        try:
            ...
            last_kind, last_status = classify_search_error(exc)

    write_error_file(
        PROVIDER_ID, CAPABILITY, kind=last_kind, status=last_status, message=failures[-1] if failures else "no SearXNG instance"
    )
