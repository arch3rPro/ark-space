...
```bash
python3 scripts/arkspace.py provider check tavily --capability web_map
```
...
```bash
python3 scripts/arkspace.py site map --provider tavily "https://docs.example.com" --output json
```
...
```bash
python3 scripts/arkspace.py site map --provider tavily "https://example.com" \
