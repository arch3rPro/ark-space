...
def classify_exception(exc):
    """Map an Exa provider exception to a (kind, status) failure tuple.

    Configuration failures (the provider is missing/not configured) map to
    ``config``; request failures carry a status and are classified through
    :func:`classify_failure`. A request that returned an unparseable body is
    classified as ``invalid-response``.
    """
    if isinstance(exc, exa_client.ProviderRequestError):
        status = exc.status
        text = str(exc).lower()
...
def main() -> int:
    ...
    try:
        ...
        provider_config.write_error_file(
            "exa", "web_search", kind=kind, status=status, message=str(exc)
        )
